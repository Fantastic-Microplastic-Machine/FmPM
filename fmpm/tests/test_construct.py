...
def test_calculate_accuracy():
    ...
    expected = .75
    assert math.isclose(ret.item(), expected), 'Failed to calculate accuracy correctly'
...
def test_train_iteration():
    ...
    data = prep.prep_data(pd.read_csv('tests/test_data/10x_labels_4.csv'), image_dir)
    image_dir = 'tests/test_data/images_10x'
    ...
    loss, acc, pred, label = construct.train_iteration(model, iterator, opt, crit, device)
    assert isinstance(loss, float), f'train_iteration error, wrong loss dtype {loss.type()}'
    assert isinstance(acc, float), f'train_iteration error, wrong acc dtype {acc.type()}'
    assert pred.size() == torch.Size([1, 2]), f'train_iteration error, prediction wrong shape/size {pred.size()}'
    assert label.size() == torch.Size([1, 2]), f'train_iteration error, label wrong shape/size {label.size()}'
...
def test_get_predictions():
    ...
    images, labels, predictions, weights, acc = construct.get_predictions(1, model, dataset)
    assert labels.size() == torch.Size([12, 2]), f'get_predictions error, incorrect labels dimensions'
    assert isinstance(acc.item(), float), f'get_predictions error, incorrect accuracy dtype'
    assert weights.size() == torch.Size([12, 2]), f'get_predictions error, incorrect weight dimensions'
    assert predictions.size() == torch.Size([12]), f'get_predictions error, incorrect predictions dimensions'
    assert images.size() == torch.Size([12, 3, 325, 325]), f'get_predictions error, incorrect image'
...
def test_load_model_from_file():
    ...
    model = test_model()
    try:
        construct.load_model_from_file('tests/test_data/test_saved_model', model)
    except Exception as e:
